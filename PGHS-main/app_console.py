...
class PGHSProcessor:
    ...
    def load_model(self):
        ...
        try:
            ...
            flops_info = metrics.calculate_flops_accurate(model_copy, input_shape=(1, 3, 256, 256), device=self.device)
            del model_copy
            print(f"  FLOPs (GFLOPs)     : {flops_info['flops']:.3f}")
            print(f"  MACs (GMACs)       : {flops_info['macs']:.3f}")
            print(f"  Parameters (M)     : {flops_info['params']:.3f}")
        except Exception:
            print(f"  FLOPs calculation failed (skipped)")
            param_count = sum(p.numel() for p in self.model.parameters()) / 1e6
            print(f"  Parameters (M)     : {param_count:.3f}")
    ...
    def preprocess_image(self, image, target_size=(256, 256)):
        """
        Preprocess image for PGHS model.
...
def process_directory(processor, input_dir, output_dir, gt_dir=None):
    ...
    Args:
        processor: PGHSProcessor instance
...
def main():
    parser = argparse.ArgumentParser(description="PGHS CLI for Underwater Image Enhancement")
    ...
    parser.add_argument('--model', type=str, default='/home/ndpthao/eject/IMPLEMENTATION/PGHS-main/models/uw_epoch_79.pth', help='Model path')
    ...
    processor = PGHSProcessor(model_path=args.model, device=device)
