#!/usr/bin/env python3
"""
PGHS ONNX Inference Script
Optimized inference using ONNX Runtime for PGHS underwater image enhancement model.

Usage:
    python inference_onnx.py --input test.jpg --output output.jpg
    python inference_onnx.py --input /path/to/dir --output /path/to/output --gt /path/to/gt
"""
...
class PGHSOnnx:
    """PGHS ONNX Inference Engine"""

    def __init__(self, onnx_path='models/uw_epoch_79.onnx', device='cpu', warmup=True):
        ...
        print(f"Loading PGHS ONNX model from {onnx_path}...")
        ...
        # Configure providers with availability checking
...
def process_directory(model, input_dir, output_dir, gt_dir=None):
    ...
    for idx, img_file in enumerate(tqdm(image_files, desc="Processing images")):
        ...
        # Skip first frame timing for FPS calculation
        if idx > 0:
    ...
    # Display results
    print("\n" + "=" * 70)
    print("BATCH ENHANCEMENT RESULTS (PGHS ONNX)")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="PGHS ONNX Inference")
    parser.add_argument('--input', type=str, required=True, help='Input image or directory')
    parser.add_argument('--output', type=str, required=True, help='Output directory')
    parser.add_argument('--gt', type=str, default=None, help='Ground truth directory (optional)')
    parser.add_argument('--model', type=str, default='models/uw_epoch_79.onnx', help='Path to ONNX model')
    parser.add_argument('--device', type=str, default='cpu', choices=['cpu', 'cuda'], help='Device (cpu/cuda)')

    args = parser.parse_args()

    model = PGHSOnnx(onnx_path=args.model, device=args.device)
