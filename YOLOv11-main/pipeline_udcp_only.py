"""
UDCP-Only UIE + YOLO Detection Pipeline
=======================================

Dedicated pipeline for Underwater Dark Channel Prior (UDCP) model.
UDCP is slow (3-4 hours for 800 images) so it runs separately.

Pipeline:
1. Run UDCP enhancement on test images
...
YOLO_MODEL = "/home/ndpthao/eject/runs/detect/train11/weights/best.pt"
DATA_YAML = DATASET_DIR / "data.yaml"

TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_BASE = BASE_DIR / f"UIE_YOLO_UDCP_{TIMESTAMP}"
OUTPUT_BASE.mkdir(parents=True, exist_ok=True)

UDCP_SCRIPT = BASE_DIR / 'UDCP-main/app_console.py'

# Import helper functions from pipeline_dcp_only
...
def run_udcp_enhancement(input_dir, output_dir):
    """Run UDCP enhancement with extended timeout"""
    print(f"\n{'='*80}\nRunning UDCP Enhancement\n{'='*80}")
    print(f"⚠️  WARNING: UDCP is very slow. Expected time: 3-4 hours for 800 images")
    print(f"Input: {input_dir}")
    print(f"Output: {output_dir}")

    if not UDCP_SCRIPT.exists():
        print(f"✗ UDCP script not found: {UDCP_SCRIPT}")
        return False
    ...
    # Prepare command
    cmd = [sys.executable, str(UDCP_SCRIPT), '--input', str(input_dir / 'images'), '--output', str(images_output_dir)]

    model_dir = UDCP_SCRIPT.parent
    timeout = 18000  # 5 hours

    print(f"\nStarting UDCP at {time.strftime('%H:%M:%S')}...")
    ...
    try:
        ...
        hours = elapsed / 3600
        print(f"\n✓ UDCP Completed in {hours:.2f} hours ({elapsed:.0f}s)")
        ...
        print(f"\n✗ UDCP TIMEOUT after {timeout/3600:.1f} hours!")
        return False
...
def main():
    ...
    # Step 2: UDCP Enhancement
    print(f"\n{'='*80}\nSTEP 2: UDCP ENHANCEMENT\n{'='*80}")
    enhanced_dir = OUTPUT_BASE / 'UDCP' / 'enhanced'
    success = run_udcp_enhancement(TEST_DIR, enhanced_dir)

    if not success:
        print("✗ UDCP enhancement failed. Exiting.")
        return

    # Step 3: YOLO on UDCP Enhanced Images
    print(f"\n{'='*80}\nSTEP 3: YOLO on UDCP Enhanced Images\n{'='*80}")
    yolo_output = OUTPUT_BASE / 'UDCP' / 'yolo'
    udcp_metrics = run_yolo_detection(YOLO_MODEL, enhanced_dir, DATA_YAML, yolo_output, save_viz=True)

    if not udcp_metrics:
        print("✗ Failed to run YOLO on UDCP enhanced images.")
        return
    ...
    udcp_viz_dir = udcp_metrics.get('visualization_dir')

    if original_viz_dir and udcp_viz_dir:
        comparison_dir = OUTPUT_BASE / 'UDCP' / 'comparisons'
        # Modify labels for UDCP
        def create_udcp_comparisons(raw_images_dir, labels_dir, enhanced_viz_dir, original_viz_dir, output_dir, class_names):
            ...
            for img_name, enhanced_file in tqdm(enhanced_files.items(), desc="Creating 3-way comparisons"):
                ...
                try:
                    ...
                    labels = ['Raw Image + Ground Truth', 'UDCP Enhanced + Detection', 'Original + Detection']
        ...
        create_udcp_comparisons(
            raw_images_dir=str(raw_images_dir),
            labels_dir=str(labels_dir),
            enhanced_viz_dir=udcp_viz_dir,
    ...
    report = [
        "# UDCP + YOLO Detection Evaluation Report",
        "",
    ...
    report.append(f"| UDCP Enhanced | {um.get('mAP50', 0):.4f} | {um.get('mAP50-95', 0):.4f} | {um.get('precision', 0):.4f} | {um.get('recall', 0):.4f} | {um.get('f1', 0):.4f} |")

    report.extend(["", "## Improvement Analysis", ""])
    improvements = {}
    for key in ['mAP50', 'mAP50-95', 'precision', 'recall', 'f1']:
        orig_val = om.get(key, 0)
        udcp_val = um.get(key, 0)
        if orig_val > 0:
            improvement = ((udcp_val - orig_val) / orig_val) * 100
            improvements[key] = improvement

    report.extend([
        "| Metric | Original | UDCP Enhanced | Improvement |",
        "|--------|----------|---------------|-------------|"
    ])
    ...
    report_file = OUTPUT_BASE / 'UDCP_YOLO_Report.md'
    with open(report_file, 'w') as f:
        f.write('\n'.join(report))
    print(f"\n✓ Report: {report_file}")

    # Save results
    results = {'Original': original_metrics, 'UDCP': udcp_metrics}
    ...
    print(f"Comparison images: {OUTPUT_BASE / 'UDCP' / 'comparisons'}")
    print(f"{'='*80}\n")
