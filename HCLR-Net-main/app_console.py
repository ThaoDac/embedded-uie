...
class HCLRNetProcessor:
    ...
    def load_model(self):
        ...
        # Extract network from Lightning module
        ...
        flops_info = metrics.calculate_flops(self.network, input_shape=(1, 3, 256, 256), device=self.device)
        ...
        model_memory = metrics.get_memory_usage(self.network, device=self.device)
...
def process_directory(processor, input_dir, output_dir, gt_dir=None):
    ...
    for img_file in tqdm(image_files, desc="Processing images"):
        ...
        memory_per_image = metrics.get_memory_usage(model=processor.network, device=processor.device)
    ...
    model_size_mb = metrics.get_model_size(processor.checkpoint_path)
    ...
    print(f"  Model Size            : {avg_metrics['model_size_mb']:.2f} MB  (checkpoint file)")
...
def process_video(processor, video_path, output_dir):
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            output_tensor = processor.network(img_tensor)
...
def main():
    parser = argparse.ArgumentParser(description="HCLR-Net CLI for Enhancement")
    ...
    processor = HCLRNetProcessor(checkpoint_path=args.checkpoint, device=device)
