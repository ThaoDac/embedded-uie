#!/usr/bin/env python3
"""
FUnIE-GAN Console Application
Underwater Image Enhancement - Command Line Interface

Usage:
    python app_console_pytorch.py --input <input_dir> --output <output_dir> [--gt <gt_dir>]

Example:
    python app_console_pytorch.py --input /path/to/input --output /path/to/output
    python app_console_pytorch.py --input /path/to/input --output /path/to/output --gt /path/to/gt
"""
...
class FUNIEGANProcessor:
    def __init__(self, model_path='/home/ndpthao/eject/IMPLEMENTATION/FUnIE-GAN-main/FUnIE-GAN-origin/PyTorch/checkpoints/FunieGAN/pghs_uie/generator_best.pth', device=None):
    ...
    def load_model(self):
        print(f"Loading FUnIE-GAN model...")
        print(f"Model path: {self.model_path}")
        print(f"Using device: {self.device}")

        self.model = funiegan.GeneratorFunieGAN().to(self.device)
    ...
    def preprocess_image(self, image):
        """Preprocess image: BGR -> RGB -> normalize -> tensor"""
        # Convert BGR to RGB
    ...
    def postprocess_tensor(self, tensor):
        """Convert model output tensor to numpy RGB array"""
        out = (tensor.squeeze().detach().cpu() * 0.5 + 0.5).numpy()
    ...
    def _process_image_core(self, image):
        ...
        output_rgb = self.postprocess_tensor(output_tensor)
    ...
    def process_image(self, image):
        """Process a single image and return enhanced result with timing"""
...
def process_directory(processor, input_dir, output_dir, gt_dir=None):
    ...
    # Calculate FLOPs info (only once for display)
    flops_info = metrics.calculate_flops_accurate(processor.model, input_shape=(1, 3, 256, 256), device=processor.device)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='FUnIE-GAN Console - Underwater Image Enhancement',
...
Examples:
  ...
  # Process with ground truth for PSNR/SSIM
  python app_console_pytorch.py --input /path/to/input --output /path/to/output --gt /path/to/gt

  # Custom model path
  python app_console_pytorch.py --input /path/to/input --output /path/to/output --model_path custom_model.pth
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Ground truth directory for PSNR/SSIM calculation (optional)'
    )

    parser.add_argument(
        '--model_path',
        type=str,
        default='/home/ndpthao/eject/IMPLEMENTATION/FUnIE-GAN-main/FUnIE-GAN-origin/PyTorch/checkpoints/FunieGAN/pghs_uie/generator_best.pth',
        help='Path to pretrained model (default: PyTorch/models/funie_generator.pth)'
    )
    ...
    if args.gt and not os.path.exists(args.gt):
        print(f"Error: Ground truth directory not found: {args.gt}")
        sys.exit(1)

    # Initialize processor
    try:
        device = torch.device(args.device if torch.cuda.is_available() else 'cpu')
        processor = FUNIEGANProcessor(model_path=args.model_path, device=device)

        # Process directory
        process_directory(processor, args.input, args.output, args.gt)
