...
def get_memory_usage(model=None):
    """
    Get model memory usage in MB (GPU or CPU).

    This measures the memory used by the model weights and buffers only,
    ...
    """
    try:
        # Try to get GPU memory usage (PyTorch only)
        if torch.cuda.is_available() and model is not None and hasattr(model, 'parameters'):
        ...
        # Fallback: estimate model size from parameters
        if model is not None:
            ...
            try:
                ...
                # Keras/TensorFlow model
                    ...
                    total_params = model.count_params() * 4  # Assume float32 (4 bytes)
                    return total_params / (1024 ** 2)
            except Exception as e:
                print(f"Error calculating model memory: {str(e)}")

        # Last resort: use process memory (less accurate)
...
def get_inference_memory_usage(use_tracemalloc=True, use_psutil=True):
    """
    Get actual memory usage during inference (including activations, tensors, etc.).

    This function measures the REAL memory consumption during inference,
    including model weights, intermediate activations, input/output tensors.

    Args:
        use_tracemalloc: Use tracemalloc for Python object tracking (more accurate)
        use_psutil: Use psutil for process memory (includes everything)

    Returns:
        dict: Memory usage information in MB
            - 'python_memory_mb': Memory tracked by Python (tracemalloc)
            - 'process_memory_mb': Total process RSS memory (psutil)
            - 'gpu_memory_mb': GPU memory if available (PyTorch CUDA)
    """
    memory_info = {
    ...
    try:
        # 1. Python memory tracking (tracemalloc)
        if use_tracemalloc and tracemalloc.is_tracing():
            current, peak = tracemalloc.get_traced_memory()
            memory_info['python_memory_mb'] = current / (1024 ** 2)
            memory_info['python_peak_mb'] = peak / (1024 ** 2)

        # 2. Process memory (psutil) - includes everything
        if use_psutil:
            process = psutil.Process(os.getpid())
            mem_info = process.memory_info()
            memory_info['process_memory_mb'] = mem_info.rss / (1024 ** 2)
            memory_info['process_vms_mb'] = mem_info.vms / (1024 ** 2)

        # 3. GPU memory (PyTorch CUDA)
        if torch.cuda.is_available():
            ...
            memory_info['gpu_peak_mb'] = torch.cuda.max_memory_allocated() / (1024 ** 2)

    except Exception as e:
        print(f"Error getting inference memory usage: {str(e)}")

    return memory_info


def measure_inference_memory(model, input_data, predict_fn=None, warmup_runs=2, measure_runs=1):
    """
    Measure actual memory consumption during model inference.

    This function provides ACCURATE memory measurements by:
    1. Running garbage collection
    2. Warming up the model
    3. Tracking memory before and during inference
    4. Measuring peak memory usage

    Args:
        model: Model instance (PyTorch or Keras/TensorFlow)
        input_data: Input tensor/array for inference
        predict_fn: Custom prediction function (optional)
                   If None, will use model.predict() or model()
        warmup_runs: Number of warmup runs before measurement (default: 2)
        measure_runs: Number of inference runs to measure (default: 1)

    Returns:
        dict: Detailed memory usage information
            - 'baseline_memory_mb': Memory before inference
            - 'peak_memory_mb': Peak memory during inference
            - 'inference_memory_mb': Additional memory used during inference
            - 'gpu_baseline_mb': GPU memory before inference (if available)
            - 'gpu_peak_mb': GPU peak memory during inference (if available)
            - 'gpu_inference_mb': Additional GPU memory during inference
    """
    memory_results = {
    ...
    try:
        ...
        # Warmup runs (to load model into memory, compile operations, etc.)
        if predict_fn is None:
            # Auto-detect predict function
            if hasattr(model, 'predict'):
                predict_fn = lambda x: model.predict(x, verbose=0)
            elif callable(model):
                predict_fn = model
            else:
                raise ValueError("Cannot determine prediction function. Please provide predict_fn.")
        ...
        if torch.cuda.is_available():
            torch.cuda.synchronize()
            gpu_baseline = torch.cuda.memory_allocated() / (1024 ** 2)
            memory_results['gpu_baseline_mb'] = gpu_baseline

        # Run inference and measure peak memory
        print(f"Measuring memory for {measure_runs} inference run(s)...")
        for _ in range(measure_runs):
            _ = predict_fn(input_data)

        # Get peak memory
        current_memory, peak_memory = tracemalloc.get_traced_memory()
        memory_results['peak_memory_mb'] = process.memory_info().rss / (1024 ** 2)
        memory_results['inference_memory_mb'] = memory_results['peak_memory_mb'] - baseline_memory

        if torch.cuda.is_available():
            torch.cuda.synchronize()
            gpu_peak = torch.cuda.max_memory_allocated() / (1024 ** 2)
            memory_results['gpu_peak_mb'] = gpu_peak
            memory_results['gpu_inference_mb'] = gpu_peak - gpu_baseline

        # Stop tracking
        tracemalloc.stop()

        print(f"✓ Memory measurement complete")
        print(f"  Baseline: {baseline_memory:.2f} MB")
        print(f"  Peak: {memory_results['peak_memory_mb']:.2f} MB")
        print(f"  Inference Memory: {memory_results['inference_memory_mb']:.2f} MB")

        if torch.cuda.is_available():
            print(f"  GPU Baseline: {gpu_baseline:.2f} MB")
            print(f"  GPU Peak: {gpu_peak:.2f} MB")
            print(f"  GPU Inference Memory: {memory_results['gpu_inference_mb']:.2f} MB")
...
def calculate_flops(image_shape, model_complexity_factor=30):
    """
    Estimate FLOPs (Floating Point Operations) in MFLOPs (legacy/simple method).
    This is a simplified estimation based on image size.
...
def calculate_flops_accurate(model, input_shape=(1, 3, 256, 256), device='cpu'):
    """
    Calculate FLOPs (Floating Point Operations) accurately using thop library.
...
def calculate_performance_metrics(start_time, end_time, image_shape, model_path='PyTorch/models', model=None,
    ...
    Args:
        ...
        inference_memory_info: Dictionary with actual inference memory measurements (optional)
    ...
    try:
        ...
        # Memory and model metrics
        # If inference_memory_info provided, use actual inference memory
        if inference_memory_info is not None:
            memory_mb = inference_memory_info.get('inference_memory_mb', 0.0)
            peak_memory_mb = inference_memory_info.get('peak_memory_mb', 0.0)
            baseline_memory_mb = inference_memory_info.get('baseline_memory_mb', 0.0)
            gpu_inference_mb = inference_memory_info.get('gpu_inference_mb', 0.0)
        else:
            # Fallback to model-only memory (old behavior)
        ...
        if inference_memory_info is not None:
            ...
            metrics['baseline_memory_mb'] = baseline_memory_mb
            metrics['gpu_inference_mb'] = gpu_inference_mb
