...
class ImageCleanModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.gradient_accumulation_steps = self.opt['train'].get('gradient_accumulation_steps', 1)
    ...
    def nonpad_test(self, img=None):
        ...
        # Check if image is too large, use tiled inference
        _, _, h, w = img.size()
        max_size = self.opt.get('val', {}).get('max_size', 512)

        if h > max_size or w > max_size:
            # Use tiled inference for large images
            self.output = self.tile_test(img, tile_size=max_size, tile_overlap=32)
        else:
            # Normal inference for small images
    ...
    def tile_test(self, img, tile_size=512, tile_overlap=32):
            ...
            for i in range(h_tiles):
                for j in range(w_tiles):
                    ...
                    try:
                        ...
                        # Accumulate output
                        output[:, :, h_start:h_end, w_start:w_end] += pred
                        count[:, :, h_start:h_end, w_start:w_end] += 1
                    except torch.cuda.OutOfMemoryError:
                        # If still OOM, skip this tile
                        print(f"OOM on tile [{i},{j}], skipping...")
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            try:
                ...
                print(f"Validation error for image {idx}: {e}")
                # tentative for out of GPU memory
                if hasattr(self, 'lq'):
                    del self.lq
