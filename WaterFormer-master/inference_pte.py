...
# Import WaterFormerExport from convert_to_pte for FLOPs calculation
try:
    from convert_to_pte import WaterFormerExport
    HAS_WATERFORMER_EXPORT = True
except ImportError:
    HAS_WATERFORMER_EXPORT = False


def calculate_pte_flops(input_shape=(1, 3, 256, 256)):
    """
    Calculate FLOPs for ExecuTorch WaterFormer model.
    Since ExecuTorch models don't support direct FLOPs counting,
    we create an equivalent PyTorch model to measure FLOPs.

    Args:
        input_shape: Input tensor shape (batch, channels, height, width)

    Returns:
        dict: FLOPs information or None if calculation fails
    """
    if not HAS_WATERFORMER_EXPORT:
        print("  ⚠ Cannot calculate FLOPs: WaterFormerExport not available")
        return None

    try:
        # Create equivalent PyTorch model for FLOPs calculation
        pytorch_model = WaterFormerExport(
            inp_channels=3,
            out_channels=3,
            dim=36,
            num_blocks=[2, 2, 2, 2],
        ...
        # Use metrics.calculate_flops_accurate for FLOPs calculation
        flops_info = metrics.calculate_flops_accurate(
            pytorch_model,
...
class WaterFormerPTEProcessor:
    def __init__(self, model_path='/home/ndpthao/eject/IMPLEMENTATION/WaterFormer-master/work_dirs/UW_WaterFormer/models/net_g_best.pte', device=None):
    ...
    def load_model(self):
        ...
        # Calculate FLOPs using equivalent PyTorch model
        print("\nModel Complexity Analysis:")
        self.flops_info = calculate_pte_flops(input_shape=(1, 3, 256, 256))
        if self.flops_info:
            print(f"  FLOPs (GFLOPs)     : {self.flops_info['flops']:.3f}")
            print(f"  MACs (GMACs)       : {self.flops_info['macs']:.3f}")
            print(f"  Parameters (M)     : {self.flops_info['params']:.3f}")
        else:
            self.flops_info = {'flops': 0, 'macs': 0, 'params': 0}
    ...
    def preprocess_image(self, image, target_size=(256, 256)):
        ...
        return tensor, rgb, (height, width)
    ...
    def _process_image_core(self, image):
        ...
        img_tensor, input_rgb, original_size = self.preprocess_image(image)
...
def process_directory(processor, input_dir, output_dir, gt_dir=None):
    ...
    print("-" * 70)
    print("MODEL COMPLEXITY (equivalent PyTorch model)")
    if processor.flops_info and processor.flops_info.get('flops', 0) > 0:
        print(f"  FLOPs (GFLOPs)        : {processor.flops_info['flops']:.3f}")
        print(f"  MACs (GMACs)          : {processor.flops_info['macs']:.3f}")
        print(f"  Parameters (M)        : {processor.flops_info['params']:.3f}")
    else:
        print("  FLOPs                 : N/A (thop library not available)")
    print("-" * 70)
...
def process_video(processor, video_path, output_dir):
    ...
    while True:
        ...
        frame_idx += 1
        start = time.time()
        img_tensor, _, original_size = processor.preprocess_image(frame)
...
def main():
    parser = argparse.ArgumentParser(description="WaterFormer ExecuTorch CLI for Enhancement")
    ...
    parser.add_argument('--model', type=str, default='/home/ndpthao/eject/IMPLEMENTATION/WaterFormer-master/work_dirs/UW_WaterFormer/models/net_g_best.pte', help='ExecuTorch model path (.pte)')
    parser.add_argument('--video', action='store_true', help='Process as video input')

    args = parser.parse_args()
    processor = WaterFormerPTEProcessor(model_path=args.model)
