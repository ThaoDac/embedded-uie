...
def calculate_niqe(img_tensor, device=None):
    ...
    try:
        ...
        if TORCH_AVAILABLE and not torch.is_tensor(img_tensor):
...
def calculate_flops(image_shape, model_complexity_factor=30):
    """
    Estimate FLOPs (Floating Point Operations) in MFLOPs.
...
def calculate_performance_metrics(start_time, end_time, image_shape, model_path='PyTorch/models'):
    """
    ...
    try:
        ...
        # Energy consumption
        energy_j, battery_wh = calculate_energy_consumption(latency)
...
def calculate_video_fps_metrics(frame_times):
    """
    Calculate video FPS metrics from frame processing times.

    Args:
        frame_times: List of processing times for each frame
    ...
    if total_frames > 1:
        frames_for_fps = frame_times[1:]
        frames_used = len(frames_for_fps)
        total_time = sum(frames_for_fps)

        if total_time > 0:
            avg_fps = frames_used / total_time
            min_fps = 1.0 / max(frames_for_fps) if max(frames_for_fps) > 0 else 0.0
            max_fps = 1.0 / min(frames_for_fps) if min(frames_for_fps) > 0 else 0.0
        else:
...
def evaluate_all_image_metrics(input_img, output_img, output_tensor=None, device=None):
    ...
    # NIQE (requires tensor input)
    if output_tensor is not None:
        try:
            metrics['niqe'] = calculate_niqe(output_tensor, device)
        except Exception as e:
